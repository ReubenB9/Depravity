...
```bash
my-application$ sam local invoke usersPutItemFunction --event events/event-post-item.json
my-application$ sam local invoke getAllItemsFunction --event events/event-get-all-items.json --profile local
my-application$ sam local invoke usersUpdateItemFunction --event events/users-event-update-item.json -n env.json
my-application$ sam local invoke usersAddUserToGroupFunction --event events/users-event-add-user-to-group.json -n env.json
my-application$ sam local invoke usersGetItemFunction --event events/users-event-add-user-to-group.json -n env.json



my-application$ sam local invoke groupsPutItemFunction --event events/groups-event-post-item.json
my-application$ sam local invoke groupsGetItemFunction --event events/groups-event-post-item.json
...
my-application$ sam local invoke loginSignUpUserFunction --event events/login-event-sign-up-user.json -n env.json
my-application$ sam local invoke loginResendConfirmationCodeFunction --event events/login-event-sign-up-user.json -n env.json
my-application$ sam local invoke loginConfirmSignUpFunction --event events/login-event-confirm-sign-up-user.json -n env.json
my-application$ sam local invoke loginAddUserPermissionsFunction --event events/login-event-confirm-sign-up-user.json -n env.json
my-application$ sam local invoke loginLoginFunction --event events/login-event-login.json -n env.json
my-application$ sam local invoke loginForgotPasswordFunction --event events/login-event-forgot-password.json -n env.json
my-application$ sam local invoke loginConfirmForgotPasswordFunction --event events/login-event-confirm-forgot-password.json -n env.json
my-application$ sam local invoke loginGenerateCertificateFunction --event events/login-event-confirm-forgot-password.json -n env.json
...
```
aws dynamodb create-table --table-name mainTable --attribute-definitions AttributeName=id,AttributeType=S --key-schema AttributeName=id,KeyType=HASH --billing-mode PAY_PER_REQUEST --endpoint-url http://127.0.0.1:8000 --profile local
```
3. Retrieve the ip address of your docker container running dynamodb local:
```
docker inspect 873fe505e1ed8157b3c0f603c00f48ad5bf6aab5018826b4ee3315fc36b3e6c9 -f  '{{range .NetworkSettings.Networks}}{{.IPAddress}}{{end}}' 
```
...
aws dynamodb put-item \
    ...
    --endpoint-url http://127.0.0.2:8000
```
7. How to scan your table for items
```
aws dynamodb scan --table-name mainTable --endpoint-url http://127.0.0.1:8000 --profile local
```
...
```bash
sam delete --stack-name Depravity
```
